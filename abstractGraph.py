...
def _preprocess_graph(app, _dir):
    ...
    app_path = Path(app)
    output_path = Path(app_path.name)
    
    with output_path.open('w') as fp, app_path.open() as fh:
        for lines in fh:
            caller = ""
            callee = []
            line = lines.split(" ==> ")
...
def selfDefined(f, _dir):
    ...
    for key in whitelist_data:
        with open(f"{key.lower()}s.txt") as fh:
            whitelist_data[key] = [l.strip('\n').strip('.') if l.startswith('.') else l.strip('\n').strip() 
                                 for l in fh]
    
    ff = abstractToClass(whitelist_data['Class'], f, _dir)
    Path(f).unlink()  # Use Path.unlink() instead of os.remove()
    whitelist_data['Package'].reverse()
    
    fam = Process(target = abstractToMode, args=(whitelist_data['Family'], ff, _dir))
    fam.start()
    pack = Process(target=abstractToMode, args=(whitelist_data['Package'], ff, _dir))
    pack.start()
    pack.join()
